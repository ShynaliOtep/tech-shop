...
class Stepper {
            ...
            if (fn === "showText") {
                ...
                for (const glyph of glyphs) {
                    if (typeof glyph === "object" && glyph !== null) {
                        charCodeRow.append(
                            this.#c("td", glyph.originalCharCode),
                        );
                ...
                decArgs.append(
                    `minMax: ${JSON.stringify(this.#simplifyArgs(minMax))}`,
                );
                decArgs.append(this.#c("br"));
    ...
    setOperatorBBoxes(bboxes, metadata) {
        let boxesContainer =
            this.pageContainer.querySelector(".pdfBugGroupsLayer");
        if (!boxesContainer) {
        ...
        const groups = Array.from({ length: bboxes.length }, (_, i) => {
            ...
            return {
                ...
                dependencies: Array.from(
                    metadata.get(i)?.dependencies ?? [],
                ).sort(),
                dependents: Array.from(dependents.get(i) ?? []).sort(),
                isRenderingOperation:
                    metadata.get(i)?.isRenderingOperation ?? false,
                idx: i,
            };
        });
    ...
    `;
        ...
        const isEmpty = (idx) =>
            !this.operatorsGroups[idx] ||
            this.operatorsGroups[idx].minX === null;
