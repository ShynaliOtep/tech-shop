        ...
        <div class="row">
            @foreach($orders as $order)
                <div class="col s12 order-wrapper">
                    <p class="white-text order-title">{{__('translations.Order')}} <span class="orange-text"><u> #{{$order->id}}</u></span></p>
                    <p class="order-date">Дата оформление {{\App\Services\Date\DatetimeService::textFormat(new DateTime($order->created_at))}}</p>
