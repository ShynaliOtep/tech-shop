    ...
    computed: {
        selectedOption() {
            const selected = this.options.find(opt =>
                typeof opt === 'object' ? opt.value === this.selectedValue : opt === this.selectedValue
            );
            return typeof selected === 'object' ? selected.label : selected || null;
        }
    },
    watch: {
        modelValue(newVal) {
